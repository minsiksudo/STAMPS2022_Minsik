...
download.file("https://figshare.com/ndownloader/files/36400467", "data_PacbioHiFi.zip")
unzip("data_PacbioHiFi.zip")

# Where is the freshly downloaded data?
list.files()
path <- "data_PacbioHiFi" # REPLACE PATH with the path to the unzipped directory containing the gzipped fastqs
...
# In typical Pacbio data (and here) the primers are sequenced, and the
...
# As always, see `?removePrimers` for more information.

# Define primer-removed/oriented filenames
...
out <- filterAndTrim(nops, filts,
                     maxEE=1, minLen=1000, maxLen=1600, ###! Make these plugin value
                     multi=TRUE)
out
...
# We'll focus in on the mock community sample, "zymo"
# First, how many ASVs are present in that sample?
in.mock <- st["zymo_sub.fastq.gz",]>0 # logical vector, i.e. TRUE/FALSE
table(in.mock)
# How many ASVs are preesnt in this sample?
...
# Challenge: Plot the relative abundances of the ASVs for each genus in the mock.